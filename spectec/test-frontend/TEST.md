...
# Nested empty iterations

Parentheses distinguish a singleton empty element from an empty outer iteration.

```sh
$ ../src/exe-spectec/main.exe nested-empty.spectec --print-il --print-no-pos --check

;; nested-empty.spectec
syntax atom =
  | A
...
syntax record =
{
  ITEMS{many : many} many
}

;; nested-empty.spectec
def $singleton(record : record) : record
  ;; nested-empty.spectec
  def $singleton({ITEMS [?()]}) = {ITEMS [?()]}

;; nested-empty.spectec
def $empty(record : record) : record
  ;; nested-empty.spectec
  def $empty({ITEMS []}) = {ITEMS []}
