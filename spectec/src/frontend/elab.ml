...
and elab_exp_notation' env tid e t : (Il.exp list * Subst.t) attempt =
  ...
  (* Parentheses delimit one element of an iteration. In particular, `(eps)`
   * at type `t?*` denotes the singleton list containing the empty option,
   * whereas bare `eps` denotes the empty list. *)
  | ParenE _, _ when is_iter_typ env t ->
    let* e' = elab_exp env e t in
